...
class Purchases_model extends Model {
	...
	public function getProductNames($term, $limit = 5) {
		$this -> db -> where("type = 'material' AND (name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR supplier1_part_no LIKE '%" . $term . "%' OR supplier2_part_no LIKE '%" . $term . "%' OR supplier3_part_no LIKE '%" . $term . "%' OR supplier4_part_no LIKE '%" . $term . "%' OR supplier5_part_no LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
		$this -> db -> limit($limit);
	...
	public function getAllProducts() {
		$q = $this -> db -> query('SELECT * FROM ' . $this->db_systems . '.' . $this->db_prefix . '_san_pham_rows');
		if ($q -> rowCount() > 0) {
	...
	public function getProductByName($name) {
		$q = $this -> db -> query('SELECT * FROM ' . $this->db_systems . '.' . $this->db_prefix . '_san_pham_rows WHERE name = "' . $name . '"');
		if ($q -> rowCount() > 0) {
			return $q -> fetch(PDO::FETCH_OBJ);
		}
		return FALSE;
	...
	public function getAllPurchaseItems($purchase_id) {
		$this -> db ->sqlreset() -> select('purchase_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.product_unit, products.tax_method, products.' . NV_LANG_DATA . '_bodytext as details, product_variants.name as variant, products.hsn_code as hsn_code, products.second_name as second_name')
		->from($this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_purchase_items as purchase_items')
		-> join('LEFT JOIN ' . $this->db_systems . '.' . $this->db_prefix . '_san_pham_rows as products ON products.id=purchase_items.product_id LEFT JOIN ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_product_variants as product_variants  ON product_variants.id=purchase_items.option_id LEFT JOIN ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_tax_rates as tax_rates ON tax_rates.id=purchase_items.tax_rate_id')
		...
		if ($q -> rowCount() > 0) {
			foreach (($q->fetchAll(5)) as $row) {
				...
				$row->quantity_balance_fomart = storehouse_number_format($row->quantity_balance,0);
				$row->real_unit_cost_fomart = storehouse_number_format($row->real_unit_cost,0);
				$row->unit_name = $this->site->getUnitByID($row->product_unit);
				$data[] = $row;
			}
			return $data;
		}
	...
	public function addPurchase($data, $items) {
		$stmt = $this -> db -> prepare('INSERT INTO ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_purchases (reference_no, date, supplier_id, warehouse_id, note, total, product_discount, order_discount_id, order_discount, total_discount, product_tax, order_tax_id, order_tax, total_tax, shipping, grand_total, paid, status, payment_status, created_by, updated_by, updated_at, attachment, payment_term, due_date, return_id, surcharge, return_purchase_ref, purchase_id, return_purchase_total, cgst, sgst, igst, puidsite, puparentid) VALUES (:reference_no, :date, :supplier_id, :warehouse_id, :note, :total, :product_discount, :order_discount_id, :order_discount, :total_discount, :product_tax, :order_tax_id, :order_tax, :total_tax, :shipping, "", "", :status, :payment_status, :created_by, :updated_by, :updated_at, :attachment, :payment_term, :due_date, 0, "", :return_purchase_ref, :purchase_id, "", :cgst, :sgst, :igst, :idsite, :parentid)');
		...
		$stmt -> bindParam(':return_purchase_ref', $data['return_purchase_ref'], PDO::PARAM_STR);
		...
		if ($stmt -> execute()) {
			...
			if ($this -> site -> getReference('orpo') == $data['reference_no']) {
				$this -> site -> updateReference('orpo');
			}
			foreach ($items as $item) {
				...
				if ($this -> Settings -> update_cost) {
					$this -> db -> query('UPDATE ' . $this->db_systems . '.' . $this->db_prefix . '_san_pham_rows SET cost =' . $item['real_unit_cost'] . 'WHERE id = ' . $item['product_id']);
				}
				if ($item['option_id']) {
					$this -> db -> query('UPDATE ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_product_variants SET cost =' . $item['real_unit_cost'] . 'WHERE id = ' . $item['option_id'] . ' AND product_id = ' . $item['product_id']);
				}

				if ($data['status'] == '4' || $data['status'] == '5') {
					$this -> updateAVCO(array('product_id' => $item['product_id'], 'warehouse_id' => $item['warehouse_id'], 'quantity' => $item['quantity'], 'cost' => $item['real_unit_cost'], 'idsite' => $data['idsite'] , 'parentid' => $data['parentid']));
				}
			}

			if ($data['status'] == '6') {
				$this -> db -> query('UPDATE ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_purchases SET return_purchase_ref =' . $data['return_purchase_ref'] . 'surcharge=' . $data['surcharge'] . 'return_purchase_total=' . $data['grand_total'] . 'return_id=' . $purchase_id . ' WHERE id=' . $data['purchase_id']);
			} 
	...
	public function updatePurchase($id, $data, $items = array()) {
		...
		if ($exc && $this -> db -> query('DELETE FROM ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_purchase_items WHERE purchase_id = ' . $id)) {
			$purchase_id = $id;
			foreach ($items as $item) {
				$item['purchase_id'] = $id;
				$item['option_id'] = !empty($item['option_id']) && is_numeric($item['option_id']) ? $item['option_id'] : NULL;
			...
			if ($data['status'] == '4' || $data['status'] == '5') {
				...
				foreach ($oitems as $oitem) {
					$this -> updateAVCO(array('product_id' => $oitem->product_id, 'warehouse_id' => $oitem->warehouse_id, 'quantity' => (0 - $oitem->quantity), 'cost' => $oitem->real_unit_cost, 'idsite' => $data['idsite'] , 'parentid' => $data['parentid']));
				}
			}
	...
	public function addPayment($data = array()) {
		$stmt = $this -> db -> prepare('INSERT INTO ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_payments (reference_no, date, purchase_id, amount, paid_by, cheque_no, cc_no, cc_holder, cc_month, cc_year, cc_type, note, created_by, type) VALUES (:reference_no, :date, :purchase_id, :amount, :paid_by, :cheque_no, :cc_no, :cc_holder, :cc_month, :cc_year, :cc_type, :note, :created_by, :type)');
	...
	public function updateAVCO($data) {
		...
		if ($wp_details = $this -> getWarehouseProductQuantity($data['warehouse_id'], $data['product_id'])) {
			...
			if (!empty($total_quantity)) {
				...
				$this -> db -> query('UPDATE ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_warehouses_products SET avg_cost =' . $avg_cost . ' WHERE product_id = ' . $data['product_id'] . ' AND warehouse_id = ' . $data['warehouse_id'] . ' AND whpidsite = ' . $data['idsite'] . ' AND whpparentid = ' . $data['parentid']);

			}
		} else {
			$this -> db -> query('INSERT INTO ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_warehouses_products (product_id, warehouse_id, avg_cost, quantity, whpidsite, whpparentid) VALUES(' . $data['product_id'] . ', ' . $data['warehouse_id'] . ',' . $data['cost'] . ',0, ' . $data['idsite'] . ', ' . $data['parentid'] . ')');
		}
	}
	public function deletePurchase($id)
    {
		...
		$this->db->query('DELETE FROM ' . $this->db_systems . '.' . $this -> db_prefix  . '_' . $this->mod_data . '_purchases  WHERE id = ' . $this->db->quote($id));
		$this->db->query('DELETE FROM ' . $this->db_systems . '.' . $this -> db_prefix  . '_' . $this->mod_data . '_purchase_items  WHERE purchase_id = ' . $this->db->quote($id));
		$this->db->query('DELETE FROM ' . $this->db_systems . '.' . $this -> db_prefix  . '_' . $this->mod_data . '_payments  WHERE purchase_id = ' . $this->db->quote($id));
		if(!empty($purchase)){
			if ($purchase->status == 4 || $purchase->status == 5) {
				foreach ($purchase_items as $oitem) {
					
					$this->updateAVCO(array('product_id' => $oitem->product_id, 'warehouse_id' => $oitem->warehouse_id, 'quantity' => (0-$oitem->quantity), 'cost' => $oitem->real_unit_cost, 'idsite' => $oitem->puiidsite , 'parentid' => $oitem->puiparentid));
					$received = $oitem->quantity_received ? $oitem->quantity_received : $oitem->quantity;
					if ($oitem->quantity_balance < $received) {
						//print_r($oitem->quantity_balance);print_r('/'.$oitem->$received);
	...
	{
		global $array_supplier_id_storehouse,$array_warehouse_id_storehouse;
		$where = '1 ';
		if (!empty($start)) {
		...
		$this->db->select('*')
			->from('' . $this->db_prefix . '_' . $this->mod_data . '_purchases')
	        ->order('id DESC')
			->where($where);
		...
		$excel->getActiveSheet()->setCellValue('D1', $this->lang['supplier_id']);
		...
		while( $view = $sth->fetch() )
		{
			...
			$view['supplier_id'] = empty($array_supplier_id_storehouse[$view['supplier_id']]['company'])? $array_supplier_id_storehouse[$view['supplier_id']]['name'] : $array_supplier_id_storehouse[$view['supplier_id']]['company'] ;
			...
			$excel->getActiveSheet()->setCellValue('D'.$numRow, $view['supplier_id']);
		...
		header('Content-Disposition: attachment; filename="Data_Purchases_' . date("d_m_Y", $start) . '-' .  date("d_m_Y", $end) . '.xls"');
