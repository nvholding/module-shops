...
class Sales extends MY_Controller
{
    ...
    public function __construct()
    {
		...
		$this -> sales_model = &load_class('Sales_model');
        ...
        $this->data['logo'] = true;
		global $db,$db_config,$nv_Request,$admin_info,$module,$module_data,$lang, $lang_global;
		$this->db = $db;
		$this->module = $module;
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
                ...
                if (!$this->Customer && !$this->Supplier && !$this->Owner && !$this->Admin && !$this->session->userdata('view_right')) {
                    $this->datatables->where('created_by', $this->session->userdata($this -> mod_data . '_user_id'));
                } elseif ($this->Customer) {
                    $this->datatables->where('customer_id', $this->session->userdata($this -> mod_data . '_user_id'));
                }
    ...
    public function shopadd($data_order = array(), $product_storehouse = array())
    {
        ...
        if ($this->form_validation->run() == true) {
        	...
        	/*
			$customer_id = $this -> sales_model -> getCompanyIdByEmail($data_order['order_email']);
						if($customer_id == FALSE) {
							$customer_id = $this -> sales_model -> addCompanyIdByEmail($data_order['order_email'], $data_order['order_name'] , $data_order['order_phone'], $data_order['order_address']);
						}*/
			...
			$date = $data_order['order_time'];
		    $biller_id = $this->admin->userid;
		    $biller = $this->admin->username;
		    $warehouse_id = $data_order['warehouse_id'];
		    $projectid = $data_order['project_id'];
		    $note= $data_order['order_note'];
		    $staff_note= $data_order['order_note'];
		    $total= 0;
		    $product_discount= 0;
		    $order_discount_id= $data_order['order_discount_id'];
		    $total_discount= $data_order['total_discount'];
		    $order_discount= $data_order['order_discount'];
		    $product_tax= 0;
		    $order_tax_id= $data_order['order_tax_id'];
		    $order_tax= $data_order['order_tax'];
		    $total_tax= $data_order['total_tax'];
		    $shipping= $data_order['shipping'];
		    $sale_status= $data_order['sale_status'];
		    $payment_status= $data_order['payment_status'];
		    $payment_term= $data_order['payment_term'];
		    $due_date= $data_order['due_date'];
		    $created_by= $data_order['created_by'];
		    $updated_by= $data_order['updated_by'];
		    $updated_at= $data_order['updated_at'];
		    $total_items= $data_order['total_items'];
			$data_order['pos'] = 0;
		    $pos= $data_order['pos'];
		    $paid= $data_order['paid'];
		    $return_id= $data_order['return_id'];
		    $surcharge= $data_order['surcharge'];
		    $attachment= $data_order['attachment'];
		    if (is_file(NV_DOCUMENT_ROOT . $attachment))     {
		        $attachment= substr($attachment, strlen(NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/'));
		    } else {
		        $attachment= '';
		    }
		    $return_sale_ref= $data_order['return_sale_ref'];
		    $sale_id= $data_order['sale_id'];
		    $return_sale_total= $data_order['return_sale_total'];
		    $rounding= $data_order['rounding'];
		    $suspend_note= $data_order['suspend_note'];
		    $api= $data_order['api'];
		    $shop= $data_order['shop'];
		    $address_id= $data_order['address_id'];
		    $reserve_id= $data_order['reserve_id'];
			...
			foreach ($product_storehouse as $pro_id => $info){
				...
				if($info['pro_storehouse_list'] !=''){
					$array_product = json_decode($info['pro_storehouse_list']);
					foreach($array_product as $products_sh)
					{
						$product = $this->site->getProductByID($products_sh->id_product);/* print_r($info); */
						$item_id = $products_sh->id_product;
						$item_type = $product->type;
						$item_code = $products_sh->product_code;
						$item_name = $product->name;
						$item_option = 0;
						$real_unit_price = storehouse_number_format($info['price'],0,'','');
						$unit_price = storehouse_number_format($info['price'],0,'','');
						$item_unit_quantity = products_shnumber_product;
						$item_serial = '';
						$item_tax_rate = 1;
						$item_discount = 0;
						$item_unit = $product->sale_unit;
						$item_quantity = $products_sh->number_product;
						if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
		                    ...
		                    $unit_price = storehouse_number_format($unit_price - $pr_discount,0,'','');
							...
							if (isset($item_tax_rate) && $item_tax_rate != 0) {
															...
															$pr_item_tax = storehouse_number_format(($item_tax * $item_unit_quantity), 4);
		                    ...
		                    $product = array(
		                        ...
		                        'unit_price' => storehouse_number_format($item_net_price + $item_tax,0,'',''),
		                        ...
		                        'subtotal' => storehouse_number_format($subtotal,0,'',''),
		                        'serial_no' => $item_serial,
		                        'real_unit_price' => $real_unit_price,
		                    );
							$products[] = $product;
		        			$total += storehouse_number_format(($item_net_price * $item_quantity),0,'','');
		                   //print_r($product);
						}
					}
				}
			...
			$data = array('date' => $date,
                ...
                'order_tax' => $order_tax,
                'total_tax' => $total_tax,
                'shipping' => storehouse_number_format($shipping,0,'',''),
                ...
                'idsite' => $data_order['idsite'],
                'parentid' => $data_order['parentid']
            );
            ...
            if ($payment_status == 3 || $payment_status == 4) {
                if ($data_order['paid_by'] == 'deposit') {
                    if ( ! $this->site->check_customer_deposit($customer_id, $data_order['amount-paid'])) {
                        $this->session->set_flashdata('error', lang("amount_greater_than_deposit"));
                        redirect($_SERVER["HTTP_REFERER"]);
                    }
                }
                if ($data_order['paid_by'] == 'gift_card') {
                    $gc = $this->site->getGiftCardByNO($data_order['gift_card_no']);
                    $amount_paying = $grand_total >= $gc->balance ? $gc->balance : $grand_total;
                    $gc_balance = $gc->balance - $amount_paying;
                    $payment = array(
                        'date' => $date,
                        'reference_no' => $data_order['payment_reference_no'],
                        'amount' => storehouse_number_format($amount_paying,0,'',''),
                        'paid_by' => $data_order['paid_by'],
                        'cheque_no' => $data_order['cheque_no'],
                        'cc_no' => $data_order['gift_card_no'],
                        'cc_holder' => $data_order['pcc_holder'],
                        'cc_month' => $data_order['pcc_month'],
                        'cc_year' => $data_order['pcc_year'],
                        'cc_type' => $data_order['pcc_type'],
                        'created_by' => $this->session->userdata($this -> mod_data . '_user_id'),
                        'note' => $data_order['payment_note'],
                        'type' => 'received',
                    ...
                    $payment = array(
                        'date' => $date,
                        'reference_no' => $data_order['payment_reference_no'],
                        'amount' => storehouse_number_format($data_order['amount-paid'],0,'',''),
                        'paid_by' => $data_order['paid_by'],
                        'cheque_no' => $data_order['cheque_no'],
                        'cc_no' => $data_order['gift_card_no'],
                        'cc_holder' => $data_order['pcc_holder'],
    ...
    public function edit($id = null)
    {
        ...
        $inv = $this->sales_model->getInvoiceByID($id);
        //if ($inv->sale_status == 6 || $inv->return_id || $inv->return_sale_ref) {
       ...
       // $this->form_validation->set_rules('sale_status', lang("sale_status"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $warehouse_id = $this->input->get_int('warehouse_id');
            $customer_id = $this->input->get_int('customer_id');
            $project_id = $this->input->get_int('project_id');
            ...
            $shipping = $this->input->get_string('shipping') ? $this->input->get_string('shipping') : 0;
			//print_r(str_replace( ',', '', $shipping));die;
			$shipping = intval(str_replace( ',', '', $shipping));
            $customer_details = $this->site->getCompanyByID($customer_id);
            $customer = !empty($customer_details->company) && $customer_details->company != '-'  ? $customer_details->company : $customer_details->name;
            //$biller_details = $this->site->getCompanyByID($biller_id);
            //$biller = !empty($biller_details->company) && $biller_details->company != '-' ? $biller_details->company : $biller_details->name;
            $note = $this->sma->clear_tags($this->input->get_title('note'));
			...
			$products_id = $this -> input -> get_array('product_id', 'post', array());
			$products_type = $this -> input -> get_array('product_type', 'post', array());
			$products_code = $this -> input -> get_array('product_code', 'post', array());
			$products_name = $this -> input -> get_array('product_name', 'post', array());
			$products_option = $this -> input -> get_array('product_option', 'post', array());
			$reals_unit_price = $this -> input -> get_array('product_real_unit_price', 'post', array());
			$units_price = $this -> input -> get_array('product_unit_price', 'post', array());
			$quantitys = $this -> input -> get_array('quantity', 'post', array());
			$serial = $this -> input -> get_array('serial', 'post', array());
			$products_tax = $this -> input -> get_array('product_tax', 'post', array());
			$products_discount = $this -> input -> get_array('product_discount', 'post', array());
			$products_unit = $this -> input -> get_array('product_unit', 'post', array());
			$products_base_quantity = $this -> input -> get_array('product_base_quantity', 'post', array());
            $i = sizeof($products_id);
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_unit_quantity = $products_unit[$r];
                $item_serial = isset($serial[$r]) ? $serial[$r] : '';
                $item_tax_rate = isset($products_tax[$r]) ? $products_tax[$r] : null;
                $item_discount = isset($products_discount[$r]) ? $products_discount[$r] : null;
                $item_unit = $products_unit[$r];
                $item_quantity = intval(str_replace( ',', '', $products_base_quantity[$r]));
				
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    $product_details = $item_type != 'manual' ? $this->sales_model->getProductByCode($item_code) : null;

                    $pr_discount = storehouse_number_format($this->site->calculateDiscount($item_discount, $unit_price),0,'','');
                    ...
                    $total += storehouse_number_format(($real_unit_price * $item_quantity),0,'','');
                }
            }
            if (empty($products)) {
                $this->form_validation->set_rules('product', $this->lang["order_items"], 'required');
            } else {
                krsort($products);
            }

            $order_discount = $this->site->calculateDiscount($this->input->get_int('order_discount', 'post', 0), ($total + $product_tax));
            $total_discount = storehouse_number_format(($order_discount + $product_discount),0,'','');
            $order_tax = $this->site->calculateOrderTax($this->input->get_int('order_tax', 'post', 0), ($total + $product_tax - $order_discount));
            $total_tax = storehouse_number_format(($product_tax + $order_tax),0,'','');
            $grand_total = storehouse_number_format(($total + $total_tax + storehouse_number_format($shipping,0,'','') - $order_discount),0,'','');
            ...
            $data = array('date' => $date,
                ...
                'total' => $total,
                'product_discount' => $product_discount,
                'order_discount_id' => $this->input->get_int('order_discount', 'post', 0),
                'order_discount' => $order_discount,
                'total_discount' => $total_discount,
                'product_tax' => $product_tax,
                'order_tax_id' => $this->input->get_int('order_tax', 'post', 0),
                'order_tax' => $order_tax,
                'total_tax' => $total_tax,
                'shipping' => storehouse_number_format($shipping,0,'',''),
                'grand_total' => $grand_total,
                ...
                'payment_term' => $payment_term,
                'due_date' => $due_date,
                'updated_by' => $this->session->userdata($this -> mod_data . '_user_id'),
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
            ...
            foreach ($inv_items as $item) {
                ...
                $row->price = storehouse_number_format($item->net_unit_price + storehouse_number_format($item->item_discount / $item->quantity),0,'','');
                $row->unit_price = $row->tax_method ? $item->unit_price + storehouse_number_format($item->item_discount / $item->quantity,0,'','') + storehouse_number_format($item->item_tax / $item->quantity,0,'','') : $item->unit_price + ($item->item_discount / $item->quantity);
    ...
    public function return_sale($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $real_unit_price = storehouse_number_format($_POST['real_unit_price'][$r],0,'','');
                $unit_price = storehouse_number_format($_POST['unit_price'][$r],0,'','');
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = storehouse_number_format((($item_net_price * $item_unit_quantity) + $pr_item_tax),0,'','');
                    ...
                    $products[] = ($product + $gst_data);
                    $total += storehouse_number_format(($item_net_price * $item_unit_quantity),0,'','');
                }
            }
            if (empty($products)) {
            ...
            $total_tax = storehouse_number_format($product_tax + $order_tax,0,'','');
            $grand_total = storehouse_number_format(($total + $total_tax + storehouse_number_format($return_surcharge) - $order_discount),0,'','');
            $data = array('date' => $date,
                'sale_id' => $id,
                ...
                'order_tax' => $order_tax,
                'total_tax' => $total_tax,
                'surcharge' => storehouse_number_format($return_surcharge,0,'',''),
                'grand_total' => $grand_total,
                'created_by' => $this->session->userdata($this -> mod_data . '_user_id'),
    ...
    public function add_payment($id = null)
    {
        ...
        $sale = $this->sales_model->getInvoiceByID($id);
        if ($sale->payment_status == 5 && $sale->grand_total == $sale->paid) {
            return false;
        }

        //$this->form_validation->set_rules('reference_no', lang("reference_no"), 'required');
        $reference_no = $this->input->get_string('reference_no', 'post', '');
        $amount_paid = $this->input->get_int('amount', 'post', 0);
        $paid_by = $this->input->get_string('paid_by', 'post', '');
        $userfile = $this->input->get_string('userfile', 'post', '');
		$sale_id = $this->input->get_int('sale_id', 'post', 0);
		$cheque_no= $this->input->get_string('cheque_no', 'post', '');
		$gift_card_no = $this->input->get_string('gift_card_no', 'post', '');
		$pcc_no = $this->input->get_string('pcc_no', 'post', '');
		$pcc_holder = $this->input->get_string('pcc_holder', 'post', '');
		$pcc_month = $this->input->get_string('pcc_month', 'post', '');
		$pcc_year = $this->input->get_string('pcc_year', 'post', '');
		$pcc_type = $this->input->get_string('pcc_type', 'post', '');
		$note = $this->input->get_string('note', 'post', '');
        if ($this->form_validation->run() == true) {
            if ($paid_by == 'deposit') {
                $sale = $this->sales_model->getInvoiceByID($sale_id);
                $customer_id = $sale->customer_id;
                if ( ! $this->site->check_customer_deposit($customer_id,$amount_paid)) {
                    return false;
                }
            } else {
                $customer_id = null;
            }
        	$publ_date = $this->input->get_string('date', 'post', '');
            ...
            $payment = array(
                'date' => $date,
                'sale_id' => $id,
                'reference_no' => $reference_no ? $reference_no : $this->site->getReference('ppay'),
                'amount' => $amount_paid,
                'paid_by' => $paid_by,
                'cheque_no' => $cheque_no,
                'cc_no' => $paid_by == 'gift_card' ?  $gift_card_no : $pcc_no,
                'cc_holder' => $pcc_holder,
                'cc_month' => $pcc_month,
                'cc_year' => $pcc_year,
                'cc_type' => $pcc_type,
                'note' => $note,
			...
			if (!nv_is_url($this->input->get_string('attachment','post', '')) and nv_is_file($this->input->get_string('attachment','post', ''), NV_UPLOADS_DIR . '/' . $this -> mod_upload) === true) {
	
				$photo = $this -> input->get_string('attachment','post', '');
    ...
    public function suggestions($term = '', $warehouse_id = NULL , $customer_id = NULL)
    {
    	if($term == ''){
    		 $this->sma->send_json(array(array('id' => 0, 'label' => 'Khong tim thay', 'value' => $term)));
    	}
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr[] = array('id' => sha1($row->id), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id,
                    'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
                $r++;
            }
    ...
    public function sell_gift_card()
    {
        ...
        $error = null;
        $gcData = $this->input->get_array('gcdata', 'get' , array());
        if (empty($gcData[0])) {
            $error = $this->lang['gcdata_error'];
        }
        if (empty($gcData[1])) {
            $error = $this->lang['card_no_error'];
        }
        ...
        if (!$error) {
            if ($this->sales_model->addGiftCard($data)) {
                $this->sma->send_json(array('result' => 'success', 'message' => $this->lang['gift_card_added']));
            }
        } else {
    ...
    public function sale_by_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
                ...
                foreach ($final as $csv_pr) {
                    ...
                    if (isset($csv_pr['code']) && isset($csv_pr['net_unit_price']) && isset($csv_pr['quantity'])) {
                        ...
                        if ($product_details = $this->sales_model->getProductByCode($csv_pr['code'])) {
                            ...
                            $item_net_price = storehouse_number_format($csv_pr['net_unit_price'],0,'','');
                            ...
                            if (isset($item_code) && isset($item_net_price) && isset($item_quantity)) {
                                ...
                                $item_net_price = storehouse_number_format(($item_net_price - $pr_discount),0,'','');
                                $pr_item_discount = storehouse_number_format(($pr_discount * $item_quantity),0,'','');
                                $product_discount += $pr_item_discount;
                                ...
                                if ($tax_details) {
                                    ...
                                    $pr_item_tax = storehouse_number_format($item_tax * $item_quantity,0,'','');
                                    if ($this->Settings->indian_gst && $gst_data = $this->gst->calculteIndianGST($pr_item_tax, ($biller_details->state == $customer_details->state), $tax_details)) {
                                        $total_cgst += $gst_data['cgst'];
                                        $total_sgst += $gst_data['sgst'];
                                        $total_igst += $gst_data['igst'];
                                ...
                                $product_tax += $pr_item_tax;
                                $subtotal = storehouse_number_format((($item_net_price * $item_quantity) + $pr_item_tax),0,'','');
                                ...
                                $product = array(
                                    ...
                                    'serial_no' => $item_serial,
                                    'unit_price' => storehouse_number_format(($item_net_price + $item_tax),0,'',''),
                                    'real_unit_price' => storehouse_number_format(($item_net_price + $item_tax + $pr_discount),0,'',''),
                                );
            ...
            $total_discount = storehouse_number_format(($order_discount + $product_discount),0,'','');
            $order_tax = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
            $total_tax = storehouse_number_format(($product_tax + $order_tax), 4);
            $grand_total = storehouse_number_format(($total + $total_tax + storehouse_number_format($shipping) - $order_discount), 4);
            $data = array('date' => $date,
                'reference_no' => $reference,
                'customer_id' => $customer_id,
                'customer' => $customer,
                ...
                'order_tax' => $order_tax,
                'total_tax' => $total_tax,
                'shipping' => storehouse_number_format($shipping),
                'grand_total' => $grand_total,
                'total_items' => $total_items,
                'sale_status' => $sale_status,
                'payment_status' => $payment_status,
	...
	public function sales_extent($warehouse_id, $order, $order_product )
    {
    	if ($this->form_validation->run() == TRUE) {
    		$date = $order['day'];
			$warehouse_id = $warehouse_id;
			if($order['company'] != '') {
				$custommer_name = $order['company'];
			}else{
				$custommer_name = $order['name'];
			}
			if($custommer_name != '')
            	$customer_id = $this -> sales_model -> getCompanyIdByEmail($order['email']);
			else{
				$customer_id = 0;
			}
			...
			/*
			if($customer_id == FALSE) {
							$customer_id = $this -> sales_model -> addCompanyIdByEmail($order['email'], $custommer_name , $order['phone'], $order['address']);
						}*/
			
			$customer_details = $this->site->getCompanyByID($customer_id);
            $biller_id = $order['user_id'];
            ...
            $biller_details = $this->site->getCompanyByID($biller_id);
            if($biller_id > 0 ) $biller = $biller_details->company != '-' ? $biller_details->company : $biller_details->name;
            ...
            $gst_data = array();
            $total_cgst = $total_sgst = $total_igst = 0;
            $i = isset($order_product) ? sizeof($order_product) : 0;
			//print_r($order_product);
			for ($r = 0; $r < $i; $r++) {
				//print_r($order_product[$r]['product_id']);
				$item_id = $order_product[$r]['product_id'];
                $item_type = $order_product['product_type'][$r];
                $item_code = "CAFE";//$order_product[$r]['product_code'];
                $item_name = $order_product[$r]['product_name'];
                $item_comment = $_POST['product_comment'][$r];
                $item_option = isset($_POST['product_option'][$r]) && $_POST['product_option'][$r] != 'false' ? $_POST['product_option'][$r] : NULL;
                $real_unit_price = storehouse_number_format($order_product[$r]['price']);
                $unit_price = storehouse_number_format($order_product[$r]['unit']);
                $item_unit_quantity = $order_product[$r]['quantity'];
                $item_serial = isset($_POST['serial'][$r]) ? $_POST['serial'][$r] : '';
                $item_tax_rate = isset($_POST['product_tax'][$r]) ? $_POST['product_tax'][$r] : NULL;
                $item_discount = isset($_POST['product_discount'][$r]) ? $_POST['product_discount'][$r] : NULL;
                $item_unit = $order_product[$r]['unit'];
                $item_quantity = $order_product[$r]['quantity'];
                //print_r($item_code); print_r($real_unit_price);  print_r($unit_price); print_r($item_quantity); 
				 if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $pr_discount = $this->site->calculateDiscount($item_discount, $unit_price);
                    $unit_price = storehouse_number_format($unit_price - $pr_discount);
                    $item_net_price = $unit_price;
                    $pr_item_discount = storehouse_number_format($pr_discount * $item_unit_quantity);
                    $product_discount += $pr_item_discount;
                    $pr_item_tax = $item_tax = 0;
                    $tax = "";
                    ...
                    $product = array(
                        ...
                        'option_id'       => $item_option,
                        'net_unit_price'  => $item_net_price,
                        'unit_price'      => storehouse_number_format($item_net_price + $item_tax),
                        'quantity'        => $item_quantity,
                        'product_unit_id' => $unit ? $unit->id : NULL,
                        'product_unit_code' => $unit ? $unit->code : NULL,
                        'unit_quantity' => $item_unit_quantity,
                        ...
                        'discount'        => $item_discount,
                        'item_discount'   => $pr_item_discount,
                        'subtotal'        => storehouse_number_format($subtotal),
                        'serial_no'       => $item_serial,
                        'real_unit_price' => $real_unit_price,
                    ...
                    $products[] = ($product + $gst_data);
                    $total += storehouse_number_format(($item_net_price * $item_unit_quantity), 4);
                }
			}
			if (empty($products)) {
                $url_back = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $this->mod_data . '&' . NV_OP_VARIABLE . '=pos';
			    $contents = nv_theme_alert($this->mod_lang['no_product'], $this->mod_lang['no_product'], 'warning', $url_back, $this->mod_lang['continue']);
            ...
            $order_discount = $this->site->calculateDiscount($this->input->get_int('discount','post', 0), ($total + $product_tax));
            $total_discount = storehouse_number_format(($order_discount + $product_discount), 4);
            $order_tax = $this->site->calculateOrderTax($this->input->get_int('order_tax', 'post', 0), ($total + $product_tax - $order_discount));
            $total_tax = storehouse_number_format(($product_tax + $order_tax), 4);
            $grand_total = storehouse_number_format(($total + $total_tax + storehouse_number_format($shipping) - $order_discount), 4);
            $rounding = 0;
            ...
            $data = array('date'  => $date,
                ...
                'total'             => $total,
                'product_discount'  => $product_discount,
                'order_discount_id' => $this->input->get_int('discount','post', 0),
                'order_discount'    => $order_discount,
                'total_discount'    => $total_discount,
                'product_tax'       => $product_tax,
                'order_tax_id'      => $this->input->get_int('order_tax', 'post', 0),
                ...
                'payment_term'      => $payment_term,
                'rounding'          => $rounding,
                'suspend_note'      => $this->input->get_title('suspend_note', 'post', ''),
                'pos'               => 1,
                'paid'              => $this->input->get_int('amount-paid', 'post', 0),
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    if (isset($_POST['amount'][$r]) && !empty($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        $amount = storehouse_number_format($_POST['balance_amount'][$r] > 0 ? $_POST['amount'][$r] - $_POST['balance_amount'][$r] : $_POST['amount'][$r],0);
		...
		if ($this->form_validation->run() == TRUE && !empty($products) && !empty($data)) {
			if ($suspend) {
				...
				if ($sale = $this->sales_model->addSales($data, $products, $payment, $did)) {
					return $sale;
				}
			}
		}
