...
/*         $q = $this->db->get('tax_rates');
    ...
    public function getTaxRateByID($id) {
     ...
     public function getAllWarehouses($idsite, $parentid, $store_id = 0) {
		$where = ' WHERE whidsite = ' . $idsite . ' AND whparentid = ' . $parentid;
    	if($store_id > 0) {
...
/*         $date = date('Y-m-d');
    ...
    public function updateReference($field) {
        $this->db->sqlreset()->select('*')->from($this->db_prefix . "_" . $this->mod_data . "_order_ref");
		$this->db->where('ref_id = 1');
   ...
   /*      if ($warehouse_id) {
    ...
    public function getPurchasedItem($clause) {
        ...
        if (!isset($clause['option_id']) || empty($clause['option_id'])) {
            $this->db->where('option_id IS  NULL AND option_id =0');
        }
        $q = $this->db->query($this->db->sql());
        if ($q->rowCount(5) > 0) {
            return $q->fetch(5);
    ...
    public function syncProductQty($product_id, $idsite, $parentid, $warehouse_id) {
        $balance_qty = $this->getBalanceQuantity($product_id, $idsite, $parentid);
        $wh_balance_qty = $this->getBalanceQuantity($product_id, $idsite, $parentid, $warehouse_id);
		//print_r($balance_qty);print_r('/'.$wh_balance_qty);die;
        if ($this->db->query('UPDATE ' . $this->db_systems . '.' . $this->db_prefix . '_san_pham_product_quantity SET quantity = ' . $balance_qty . ' WHERE pid =' . $product_id . ' AND idsite = ' . $idsite . ' AND parentid = ' . $parentid )) {
    ...
    private function getBalanceQuantity($product_id,  $idsite = 0 , $parentid = 0, $warehouse_id = NULL) {
        
        $where = 'product_id = '. $product_id .' AND quantity_balance !=0 AND puiidsite = ' . $idsite . ' AND puiparentid = ' . $parentid;
        if ($warehouse_id) {
            $where .= ' AND warehouse_id = ' . $warehouse_id;
        }
    ...
    public function calculateAVCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
        ...
        if ($pis = $this->/($product_id, $warehouse_id, $option_id)) {
            $cost_row = array();
    ...
    public function syncQuantity($sale_id = NULL, $purchase_id = NULL, $oitems = NULL, $product_id = NULL) {
    	...
    	if ($sale_id) {
            ...
            foreach ($sale_items as $item) {
                if ($item->product_type == 'standard') {
                    $this->syncProductQty($item->product_id, $item->saiidsite, $item->saiparentid, $item->warehouse_id);
            ...
            foreach ($purchase_items as $item) {
                ...
                $this->syncProductQty($item->product_id, $item->puiidsite, $item->puiparentid, $item->warehouse_id);
                if (isset($item->option_id) && !empty($item->option_id)) {
                    $this->syncVariantQty($item->option_id, $item->warehouse_id, $item->product_id);
                }
			...
			$warehouses = $this->getAllWarehouses($global_config['idsite'],$global_config['parentid']);
            foreach ($warehouses as $warehouse) {
                $this->syncProductQty($product_id, $global_config['idsite'],$global_config['parentid'], $warehouse->id);
    ...
    public function getAllSaleItems($sale_id) {
         $q = $this->db->query('SELECT * FROM ' . $this->db_systems . '.' . $this->db_prefix . '_' . $this->mod_data . '_sale_items WHERE sale_id = ' . $sale_id);
        if ($q->rowCount() > 0) {
            return $q->fetchAll(5);
		}
        return FALSE;
