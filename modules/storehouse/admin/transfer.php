...
$row['store_id'] = $_SESSION[$module_data . '_store_id'];
 ...
 if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['idiste'] = $global_config['idsite'];
    $row['parentid'] = $site_parent;
	$i=0;
	foreach ($row['product_id'] as $product_id) {
		$quantity = $transfer->site->getWarehouseProducts($product_id,$row['warehouse_id']);
		
		if((int) $row['product_base_quantity'][$i] >  (int) $quantity['quantity']){
			$error[] = $lang_module['product_not_enough'];
		}
		$i++;
	}
    if (empty($row['date'])) {
    ...
    }elseif($row['warehouse_id_new'] == $row['warehouse_id']){
    	$error[] = $lang_module['warehouse_new_same_old'];
    }
	
     if (empty($error)) {
     		if($row['warehouse_id_new']>0){
     			 if (empty($row['id'])) {
                $row['order_discount'] = '';
                $row['paid'] = '0.0000';
                $row['created_by'] = 0;
            ...
            if ($exc) {
                ...
                nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=transfer_list' );
            }
     	}
           
    } 
} elseif ($row['id'] > 0) {
    $row = $db->query('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_transfers WHERE id=' . $row['id'])->fetch();
	$row['product_id'] = 0;
...
$xtpl = new XTemplate('transfer.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
$xtpl->assign('STORE_SESSION_2', $_SESSION[$module_data . '_store_id']);
$xtpl->assign('OP', $op);
	...
	foreach ($array_store_storehouse as $value) {
	    $xtpl->assign('STORE2', array(
	        'key' => $value['store_id'],
	        'title' => $value['name'],
	        'selected' => ($value['store_id'] == $row['store_id']) ? ' selected="selected"' : ''
	    ));
		$xtpl->parse('main.store.s2loop');
	...
	$num = $i;
	$xtpl->assign('products_transfer_total', $num);
	$xtpl->assign('ROW', $row);
