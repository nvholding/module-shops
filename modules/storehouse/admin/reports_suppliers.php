...
if (!$nv_Request->isset_request('id', 'post,get')) {
	...
	$id = $nv_Request->get_int('id', 'post, get', 0);
	$supplier_view=$rpproduct->supplier_report($id);
}
//quantity of sales
...
if ($show_view) {
    ...
    foreach ($suppliers as $key => $supplier) {
       $supplier['link_view'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=reports_suppliers&id=' .  $supplier['id'];
		$supplier['total_format'] = storehouse_number_format( $supplier['total'] ,0);
		$supplier['paid_format'] = storehouse_number_format( $supplier['paid'] ,0);
		$supplier['balance_format'] = storehouse_number_format( $supplier['balance'] ,0);
		$supplier['grand_total_fomart'] = storehouse_number_format( $supplier['grand_total'],0);
	...
	$xtpl->assign('total_amount', storehouse_number_format($supplier_view['purchases']['total_amount'],0));
	$xtpl->assign('paid', storehouse_number_format($supplier_view['purchases']['paid'],0));
	$xtpl->assign('total_paid', storehouse_number_format($supplier_view['purchases']['total_amount']-$supplier_view['purchases']['paid'],0));
	$xtpl->assign('total_purchases', storehouse_number_format($supplier_view['total_purchases'],0));
	
	
	$xtpl->parse('main.suppliers_view');
}
